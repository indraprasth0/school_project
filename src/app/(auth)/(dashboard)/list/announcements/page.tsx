...
//                     <FormModal table="announcement" data={item} type="update" id={item.id} visible={["admin"]} />
//                     <FormModal table="announcement" data={item} type="delete" id={item.id} visible={["admin"]} />
//                 </div>
//             </td>
...
//             {/* <TableTitleModal table="announcement" /> */}
...
import { AnnouncementsList } from "@/types";
...
const AnnouncementsPageList = () => {
    ...
    const {
        ...
        setGlobalFilter,
        // refetch,        
    } = useTableData<AnnouncementsList>({ endpoint });
...
const [columnOrder, setColumnOrder] = useState<string[]>(() =>
    ...
    const transformAnnouncements = (announcements: AnnouncementsList[]): Record<string, string | number>[] => {
        return announcements.map((announcement) => ({
            ...
            //     ? new Date(announcement.startTime).toLocaleTimeString("en-GB", {
            //         hour: "2-digit",
            //         minute: "2-digit",
            //         hour12: false,             
            //     })
            ...
            //     ? new Date(announcement.endTime).toLocaleTimeString("en-GB", {
            //         hour: "2-digit",
            //         minute: "2-digit",
            //         hour12: false,                       
            //     })
            //     : "N/A",
            class: typeof announcement.class === "object" && "name" in announcement.class ? String(announcement.class.name) : "N/A",
            classId: typeof announcement.class === "object" && "_id" in announcement.class ? String(announcement.class._id) : "N/A",                        
            // teacher: typeof announcement.lesson === "object" && "teacher" in event.lesson && event.lesson.teacher && typeof event.lesson.teacher === "object" && "user" in event.lesson.teacher && event.lesson.teacher.user && typeof event.lesson.teacher.user === "object" && "firstName" in event.lesson.teacher.user && "lastName" in event.lesson.teacher.user ? String(`${event.lesson.teacher.user.firstName} ${event.lesson.teacher.user.lastName}`) : "N/A",
    ...
    const formattedAnnouncementsData = useMemo(() => {
        return AnnouncementsData ? transformAnnouncements(AnnouncementsData) : [];
    }, [AnnouncementsData]);
